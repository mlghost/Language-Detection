...
def calc_prob(text, model, count, context_size=10000):
    ...
    for i in range(2, len(ntext) - 2):
        if ntext[i] != '$':
            if ntext[i] in model.keys():
                if ntext[i - 1] in model[ntext[i]].keys():
                    if ntext[i - 2] in model[ntext[i]][ntext[i - 1]].keys():
                        if model[ntext[i]][ntext[i - 1]][ntext[i - 2]] == 0:
                            p += np.log(
                                (model[ntext[i]][ntext[i - 1]][ntext[i - 2]] + 1) / (
                                        count[ntext[i - 2] + ntext[i - 1]] + 26 ** 2))
                        else:
                            p += np.log(
                                model[ntext[i]][ntext[i - 1]][ntext[i - 2]] / count[ntext[i - 2] + ntext[i - 1]])
...
en_table = {letter: {letter: {letter: 0 for letter in en_letters} for letter in en_letters} for letter in en_letters}
es_table = {letter: {letter: {letter: 0 for letter in es_letters} for letter in es_letters} for letter in es_letters}

for i in range(2, len(ngram_en_corpus) - 2):
    if ngram_en_corpus[i] != '$':
        en_table[ngram_en_corpus[i]][ngram_en_corpus[i - 1]][ngram_en_corpus[i - 2]] += 1

for i in range(2, len(ngram_es_corpus) - 2):
    if ngram_es_corpus[i] != '$':
        es_table[ngram_es_corpus[i]][ngram_es_corpus[i - 1]][ngram_es_corpus[i - 2]] += 1

count_table_en = {v: ngram_en_corpus.count(v) for v in get_comb(en_letters)}
count_table_es = {v: ngram_es_corpus.count(v) for v in get_comb(es_letters)}
